...
$prependUrl = '/eduadmin/activity/log/';
...
if (Request::is('activity/cleared')) {
    $prependUrl = '/eduadmin/activity/cleared/log/';
}
    ...
    <table class="table table-striped table-condensed table-sm @if(config('LaravelLogger.enableDrillDown') && $hoverable) table-hover @endif data-table">
        <thead>
            <tr>
                ...
                <th>
                    <i class="fas fa-school fa-fw" aria-hidden="true"></i>
                    School
                </th>
                <th>
        ...
        <tbody>
            @foreach($activities as $activity)
                <tr @if($drilldownStatus && $hoverable) class="clickable-row" data-href="{{ url($prependUrl . $activity->id) }}" title="{{trans('LaravelLogger::laravel-logger.tooltips.viewRecord')}}" @endif >
                    <td>
                        <small>
                            @if($hoverable)
                                {{ $activity->id }}
                    ...
                    <td title="{{ $activity->created_at->format('m/d/Y - g:i:s a') }}" data-toggle="tooltip">
                        {{ $activity->timePassed }}
                    </td>
                    <td>
                        {{ $activity->description }}
                    </td>
                    <td>{{ $activity->userDetails->school->name ?? '-' }}</td>
                    <td>
                        @php
                            switch ($activity->userType) {
                                case trans('LaravelLogger::laravel-logger.userTypes.registered'):
                                    $userTypeClass = 'success';
                                    $userLabel = $activity->userDetails->name ?? 'Laravel Update';
                                    $userId = $activity->userDetails->id ?? 'Laravel Update';
                                    break;

                                case trans('LaravelLogger::laravel-logger.userTypes.crawler'):
                                    $userTypeClass = 'danger';
                                    $userLabel = 'Crawler'; //$activity->userType;
                                    break;

                                case trans('LaravelLogger::laravel-logger.userTypes.guest'):
                                default:
                                    $userTypeClass = 'warning';
                                    $userLabel = $activity->userType ?? 'Laravel Update';
                                    break;
                            }
                        @endphp
                        <span class="badge badge-{{$userTypeClass}}" data-toggle="tooltip" title="{{ (isset($userId) && $userLabel !== 'Guest') ? $userId : '' }}">
                            {{$userLabel}}
                        </span>
                    </td>
                    <td>{{ $activity->userDetails->role->name ?? '-' }}</td>
                    <td>
                        @php
